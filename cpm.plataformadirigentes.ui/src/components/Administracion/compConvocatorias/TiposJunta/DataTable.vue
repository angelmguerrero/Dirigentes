<template>
    <div>
        ...
        <b-card-text>
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button
                            v-if="$store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Descargar')
                                || $store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="TiposdeJunta.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button
                            v-if="$store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Crear')
                                || $store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Todos')"
            ...
            <div class="overflow-auto">
                ...
                <b-table outlined small hover fixed id="my-table" no-border-collapse :per-page="perPage"
                    ...
                    <template #cell(estatusTipoJunta)="data">
                        <b-icon-bookmark-check-fill variant="success"
                            v-if="data.item.estatusTipoJunta"></b-icon-bookmark-check-fill>
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Editar')
                                    || $store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Todos')"
                                class="action-item mx-1" size="sm" variant="primary"
                                @click="getRowData(data.item.idTipoJunta)">
                            </b-icon-pencil-square>

                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('Tipodejuntaoconvocatoria', 'Todos')) && data.item.estatusTipoJunta"
                                class="action-item mx-1" size="sm" variant="danger"
                                @click="showDeleteModal(data.item.idTipoJunta)">
                            </b-icon-trash-fill>
                        </b-row>
                    </template>
                </b-table>
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nuevo Tipo de Junta" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Tipo de Junta" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compConvocatorias/TiposJunta/CreateForm.vue";
import EditForm from "@/components/Administracion/compConvocatorias/TiposJunta/EditForm.vue";
import DeleteModal from "@/components/Administracion/compConvocatorias/TiposJunta/DeleteModal.vue";
...
export default {
    ...
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Tipo de Junta', sortable: true },
                { key: 'estatusTipoJunta', label: 'Estatus', sortable: true },
                'acciones'
            ],
            items: [],
            id: 0,
            ...
            currentPage: 1,
            json_fields: {
                "Tipo de Junta": "descripcion",
                "Estatus": {
                    field: "estatusTipoJunta",
                    callback: (value) => {
                        if (value == true) {
                            value = "activo"
                        } else {
    ...
    methods: {
        filterTable(row, filter) {
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusTipoJunta == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusTipoJunta == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoTipoJunta/Consultar')
                .then(response => {
