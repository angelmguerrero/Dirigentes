<template>
    <div>
            ...
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Descargar')
                            || $store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="TiposBaja.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Crear')
                            || $store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Todos')"
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Editar')
                                    || $store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Todos')"
                            ...
                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('TipoBaja', 'Todos')) && data.item.estatusTipoBaja"
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nuevo Tipo de Baja" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Tipo de Baja" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compDirigentes/TipoBaja/CreateForm.vue";
import EditForm from "@/components/Administracion/compDirigentes/TipoBaja/EditForm.vue";
import DeleteModal from "@/components/Administracion/compDirigentes/TipoBaja/DeleteModal.vue";
...
export default {
    ...
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Tipo Baja', sortable: true },
                { key: 'estatusTipoBaja', label: 'Estatus' },
                'acciones'
            ],
            items: [],
            id: 0,
            ...
            currentPage: 1,
            json_fields: {
                "Tipo Baja": "descripcion",
                "Estatus": {
                    field: "estatusTipoBaja",
                    callback: (value) => {
                        if (value == true) {
                            value = "activo"
                        } else {
    ...
    methods: {
        filterTable(row, filter) {
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusTipoBaja == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusTipoBaja == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoTipoBaja/Consultar')
                .then(response => {
