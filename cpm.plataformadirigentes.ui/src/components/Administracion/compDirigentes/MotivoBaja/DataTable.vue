<template>
    <div>
        ...
        <b-card-text>
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Descargar')
                            || $store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="MotivosBaja.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Crear')
                            || $store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Todos')"
            ...
            <div class="overflow-auto">
                ...
                <b-table outlined small hover fixed id="my-table" no-border-collapse :per-page="perPage"
                    ...
                    <template #cell(estatusMotivoBaja)="data">
                        <b-icon-bookmark-check-fill variant="success"
                            v-if="data.item.estatusMotivoBaja"></b-icon-bookmark-check-fill>
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Editar') || $store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Todos')"
                                class="action-item mx-1" size="sm" variant="primary"
                                @click="getRowData(data.item.idMotivoBaja)">
                            </b-icon-pencil-square>

                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('MotivoBaja', 'Todos')) && data.item.estatusMotivoBaja"
                                class="action-item mx-1" size="sm" variant="danger"
                                @click="showDeleteModal(data.item.idMotivoBaja)">
                            </b-icon-trash-fill>
                        </b-row>
                    </template>
                </b-table>
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nuevo Motivo de Baja" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Motivo de Baja" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compDirigentes/MotivoBaja/CreateForm.vue";
import EditForm from "@/components/Administracion/compDirigentes/MotivoBaja/EditForm.vue";
import DeleteModal from "@/components/Administracion/compDirigentes/MotivoBaja/DeleteModal.vue";
...
export default {
    ...
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Motivo Baja', sortable: true },
                { key: 'estatusMotivoBaja', label: 'Estatus' },
                'acciones'
            ],
            items: [],
            id: 0,
    ...
    methods: {
        filterTable(row, filter) {
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusMotivoBaja == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusMotivoBaja == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoMotivoBaja/ConsultarMotivosDt')
                .then(response => {
