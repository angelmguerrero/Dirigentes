<template>
    <div>
        ...
        <b-card-text>
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Genero', 'Descargar')
                            || $store.getters['permisos/getPermisosByModulo']('Genero', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="Generos.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Genero', 'Crear')
                            || $store.getters['permisos/getPermisosByModulo']('Genero', 'Todos')"
            ...
            <div class="overflow-auto">
                ...
                <b-table outlined small hover fixed id="my-table" no-border-collapse :per-page="perPage"
                    ...
                    <template #cell(estatusGenero)="data">
                        <b-icon-bookmark-check-fill variant="success"
                            v-if="data.item.estatusGenero"></b-icon-bookmark-check-fill>
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('Genero', 'Editar') || $store.getters['permisos/getPermisosByModulo']('Genero', 'Todos')"
                                class="action-item mx-1" size="sm" variant="primary"
                                @click="getRowData(data.item.idGenero)">
                            </b-icon-pencil-square>

                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('Genero', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('Genero', 'Todos')) && data.item.estatusGenero"
                                class="action-item mx-1" size="sm" variant="danger"
                                @click="showDeleteModal(data.item.idGenero)">
                            </b-icon-trash-fill>
                        </b-row>
                    </template>
                </b-table>
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nuevo Genero" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Genero" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compDirigentes/Genero/CreateForm.vue";
import EditForm from "@/components/Administracion/compDirigentes/Genero/EditForm.vue";
import DeleteModal from "@/components/Administracion/compDirigentes/Genero/DeleteModal.vue";
...
export default {
    ...
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Genero', sortable: true },
                { key: 'estatusGenero', label: 'Estatus', sortable: true },
                'acciones'
            ],
            items: [],
            id: 0,
    ...
    methods: {
        filterTable(row, filter) {
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusGenero == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusGenero == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoGenero/Consultar')
                .then(response => {
