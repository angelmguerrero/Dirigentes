<template>
    <div>
        ...
        <b-card-text>
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Descargar')
                            || $store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="EstadosCiviles.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Crear')
                            || $store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Todos')"
            ...
            <div class="overflow-auto">
                ...
                <b-table outlined small hover fixed id="my-table" no-border-collapse :per-page="perPage"
                    ...
                    <template #cell(estatusEstadoCivil)="data">
                        <b-icon-bookmark-check-fill variant="success"
                            v-if="data.item.estatusEstadoCivil"></b-icon-bookmark-check-fill>
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Editar') || $store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Todos')"
                                class="action-item mx-1" size="sm" variant="primary"
                                @click="getRowData(data.item.idEstadoCivil)">
                            </b-icon-pencil-square>

                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('Estadocivil', 'Todos')) && data.item.estatusEstadoCivil"
                                class="action-item mx-1" size="sm" variant="danger"
                                @click="showDeleteModal(data.item.idEstadoCivil)">
                            </b-icon-trash-fill>
                        </b-row>
                    </template>
                </b-table>
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nuevo Estado Civil" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Estado Civil" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compDirigentes/EstadoCivil/CreateForm.vue";
import EditForm from "@/components/Administracion/compDirigentes/EstadoCivil/EditForm.vue";
import DeleteModal from "@/components/Administracion/compDirigentes/EstadoCivil/DeleteModal.vue";
...
export default {
    ...
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Estado Civil', sortable: true },
                { key: 'estatusEstadoCivil', label: 'Estatus', sortable: true },
                'acciones'
            ],
            items: [],
            id: 0,
    ...
    methods: {
        filterTable(row, filter) {
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusEstadoCivil == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusEstadoCivil == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoEstadoCivil/Consultar')
                .then(response => {
