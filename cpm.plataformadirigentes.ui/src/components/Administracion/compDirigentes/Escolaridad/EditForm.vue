<template>
    <validation-observer ref="observer" v-slot="{ handleSubmit }">
        <b-form class="mt-auto" @submit.stop.prevent="handleSubmit(update)">
            ...
            <b-row>
                <b-col cols="12">
                    <validation-provider name="Escolaridad" :rules="{ required: true, max: 100 }" v-slot="validationContext">
                        <b-form-group id="descripcion" label="Escolaridad" label-for="descripcion">


                            <b-form-input id="descripcion" type="text" v-model="escolaridad.descripcion"
            ...
            <b-row>
                <b-col cols="12">
                    <b-form-checkbox id="estatus_escolaridad" v-model="escolaridad.estatusEscolaridad"
                        name="escolaridad-status">
                        Escolaridad es Activo
                    </b-form-checkbox>
                </b-col>
...
export default {
    ...
    methods: {
        ...
        getByID() {
            ...
            HTTP.post('api/CatalogoEscolaridad/RegistrobyId', parametros).then(responseApi => {
                if (responseApi.status == 401)
                    throw new Error(this.MensajeError(this.$messageConstants('apiError')));

                if (responseApi.status == 200)
                    this.escolaridad = responseApi.data;
        ...
        update() {
            ...
            HTTP.post('api/CatalogoEscolaridad/Update', parametros).then(responseApi => {
