<template>
    <div>
        ...
        <b-card-text>
            <b-row>
                ...
                <b-col class="col-4">
                    ...
                    <b-button-group size="sm">
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Descargar')
                            || $store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Todos')"
                            variant="outline-success" id="export-btn">
                            <download-excel :data="items" name="Escolaridades.xls" type="xls" :fields="json_fields">
                                <i class="fa-solid fa-file-excel"></i> Exportar
                            </download-excel>
                        </b-button>
                        <b-button v-if="$store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Crear')
                            || $store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Todos')"
            ...
            <div class="overflow-auto">
                ...
                <b-table outlined small hover fixed id="my-table" no-border-collapse :per-page="perPage"
                    ...
                    <template #cell(estatusEscolaridad)="data">
                        <b-icon-bookmark-check-fill variant="success"
                            v-if="data.item.estatusEscolaridad"></b-icon-bookmark-check-fill>
                    ...
                    <template v-slot:cell(acciones)="data">
                        <b-row>
                            <b-icon-pencil-square
                                v-if="$store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Editar') || $store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Todos')"
                                class="action-item mx-1" size="sm" variant="primary"
                                @click="getRowData(data.item.idEscolaridad)">
                            </b-icon-pencil-square>

                            <b-icon-trash-fill
                                v-if="($store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Eliminar')
                                    || $store.getters['permisos/getPermisosByModulo']('Escolaridad', 'Todos')) && data.item.estatusEscolaridad"
                                class="action-item mx-1" size="sm" variant="danger"
                                @click="showDeleteModal(data.item.idEscolaridad)">
                            </b-icon-trash-fill>
                        </b-row>
                    </template>
                </b-table>
        ...
        <!--Modal for add-->
        <b-modal ref="create-modal" hide-footer title="Nueva escolaridad" size="md" header-bg-variant="success"
        ...
        <!--Modal for updating-->
        <b-modal ref="edit-modal" hide-footer title="Editar Escolaridad" size="md" header-bg-variant="success"
            header-text-variant="light" body-bg-variant="light" no-stacking>
...
import CreateForm from "@/components/Administracion/compDirigentes/Escolaridad/CreateForm.vue";
import EditForm from "@/components/Administracion/compDirigentes/Escolaridad/EditForm.vue";
import DeleteModal from "@/components/Administracion/compDirigentes/Escolaridad/DeleteModal.vue";
import { HTTP } from '@/plugins/axios'


export default {
    name: "dtEscolaridades",
    data() {
        return {
            fields: [
                { key: 'descripcion', label: 'Escolaridad', sortable: true },
                { key: 'estatusEscolaridad', label: 'Estatus' },
                'acciones'
            ],
            items: [],
            id: 0,
    ...
    methods: {
        ...
        filterTable(row, filter) {
            ...
            if (
                ...
                ||
                ('activo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusEscolaridad == true)
                ||
                ('inactivo'.toLowerCase().includes(filter.toLowerCase()) && row.estatusEscolaridad == false && filter.toLowerCase().substring(0, 2) == 'in')
            ) {
                return true;
            } else {
                return false;
        ...
        getData() {
            HTTP
                .get('api/CatalogoEscolaridad/Consultar')
                .then(response => {
